...
1. Run `bin/setup-hooks` from the `photos_store_app` directory to install the pre-commit hook
2. The pre-commit hook will run `rails test` before each commit
3. If tests fail, the commit will be aborted
4. To bypass the pre-commit hook, use `git commit --no-verify`
